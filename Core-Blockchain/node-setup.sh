...
task6_x402(){
  # Setup x402 native payments - TASK 6.5
  log_wait "Setting up x402 native payments system" && progress_bar
  cd ../
  
  # Run the x402 setup script (skip the build since we just did it)
...
EOF
    ...
    if [ ! -f "test-x402.sh" ]; then
      log_wait "Creating x402 test utilities"
...
EOF
    ...
    log_error "x402 setup script not found, skipping x402 configuration"
  fi
}
...
createValidator(){
  # Run the core setup tasks for validators
  task1
  task2
  task3
  task4
